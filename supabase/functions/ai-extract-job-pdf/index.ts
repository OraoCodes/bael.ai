...
serve(async (req) => {
  ...
  try {
    ...
    if (file.size > MAX_PDF_BYTES) {
      return new Response(JSON.stringify({ error: "PDF must be under 4MB" }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    const systemPrompt = `You are an expert HR professional. Extract the job posting details from the provided PDF document.
Return ONLY a valid JSON object — no markdown fences, no explanations, no text outside the JSON.

JSON schema:
{
  "title": "job title from the document (required)",
  "description": "job description 200-400 words, plain text with \\n line breaks, covering: role overview, key responsibilities, required qualifications",
  ...
  "skills": ["array of up to 8 specific lowercase skills extracted from the document, e.g. react, typescript, figma"]
}`;
      ...
      body: JSON.stringify({
        ...
        messages: [{
          ...
          content: [
            ...
            {
              type: "text",
              text: "Extract the job details from this document and return them as JSON following the schema in your instructions.",
            },
          ],
        }],
      }),
    ...
    console.error("ai-extract-job-pdf error:", error);
