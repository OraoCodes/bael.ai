      ...
      <PageHeader
        ...
        extra={
          <div className="flex items-center gap-2">
            <Button
              variant="outline"
              onClick={() => router.push(`/w/${workspace.slug}/candidates/search`)}
            >
        ...
        <div className="flex flex-wrap items-center gap-1.5 mb-3">
          {search && (
            <Badge variant="secondary" className="gap-1 pr-1">
              Search: {search}
              <button type="button" onClick={() => { setSearch(''); setPage(1) }} className="ml-0.5 hover:text-foreground">
                <X className="h-3 w-3" />
              </button>
            </Badge>
          )}
          {source && (
            <Badge variant="secondary" className="gap-1 pr-1">
              Source: {sourceLabel}
              <button type="button" onClick={() => { setSource(''); setPage(1) }} className="ml-0.5 hover:text-foreground">
                <X className="h-3 w-3" />
              </button>
            </Badge>
          )}
          {seniority && (
            <Badge variant="secondary" className="gap-1 pr-1">
              ...
              <button type="button" onClick={() => { setSeniority(''); setPage(1) }} className="ml-0.5 hover:text-foreground">
                <X className="h-3 w-3" />
              </button>
            </Badge>
          )}
