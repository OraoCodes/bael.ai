...
import { useParseResume, type AiParsedResume } from '@/lib/queries/candidates'
...
interface ResumeUploadPanelProps {
  onExtracted: (values: AiParsedResume, file: File) => void
}

export function ResumeUploadPanel({ onExtracted }: ResumeUploadPanelProps) {
  ...
  const handleExtract = async () => {
    if (!file || parseMutation.isPending) return
    try {
      const result = await parseMutation.mutateAsync(file)
      onExtracted(result, file)
    } catch {
      toast.error('Could not extract details — try a different file or enter manually.')
    }
  }
    ...
    <div className="space-y-4">
      <div className="flex items-center gap-2.5">
        ...
        <div>
          ...
          <p className="text-[11.5px] text-zinc-400 leading-snug">
            PDF only · max 4MB — AI will extract the candidate details.
          </p>
        </div>
      </div>
      ...
      <div className="flex items-center justify-end">
        <button
          ...
          disabled={!file || !!fileError || parseMutation.isPending}
